...
function countVowels(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    if (vowels.includes(lowerStr[i])) count += 1;
  }
  return count;
}
...
function isPalindrome(str) {
  ...
  const leftToRight = words.split('').join('');
  const rightToLeft = words.split('').reverse().join('');
...
function encodeToRot13(str) {
  ...
  const isLetter = (char) => table.includes(char.toLowerCase());
  const cipherer = (char) => {
    if (!isLetter(char)) return char;
    const isLowerCase = char === char.toLowerCase();
    const swapIndex = (table.indexOf(char.toLowerCase()) + 13) % 26;
    const swapChar = isLowerCase
      ? table.charAt(swapIndex)
      : table.charAt(swapIndex).toUpperCase();
    return swapChar;
  };
  return [...str].map(cipherer).join('');
}
